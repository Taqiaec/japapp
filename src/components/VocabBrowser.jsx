...
import { getTodayString } from "../utils/srs";
...
export default function VocabBrowser({ words: initialWords }) {
  ...
  function handleAdd(word) {
    updateWord(word.id, { status: "learning", dueDate: today });
    setWords((prev) =>
      prev.map((w) =>
        w.id === word.id ? { ...w, status: "learning", dueDate: today } : w
      )
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pt-6 pb-12">
      ...
      {/* Word list */}
      {paginated.length === 0 ? (
        <p className="text-center text-slate-500 py-16">No words match your filters</p>
      ) : (
        <>
            ...
            {paginated.map((w) => (
              ...
              >
                ...
                {w.status === "new" ? (
                  <button
                    onClick={() => handleAdd(w)}
                    className="shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 transition-colors"
                  >
                    + Add
                  </button>
                ) : (
                  <span className="shrink-0 px-3 py-1.5 text-xs font-semibold text-green-400">
                    ✓ Added
                  </span>
                )}
              </div>
